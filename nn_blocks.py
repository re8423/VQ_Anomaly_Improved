...
#Taken from https://github.com/snavalm/lsr_mood_challenge_2020/blob/master/nn_blocks.py
...
class MaskedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, mask_type, stride = 1, v_gated = False, cond_channels = None):
        ...
        self.weight = nn.Parameter(torch.randn((out_channels, in_channels, self.weight_size)) * 0.01,
                                   requires_grad=True) #non-masked elements (i.e. non-zeros of filter)
        self.register_buffer('rest_of_filter',torch.zeros((out_channels, in_channels,
                                                           kernel_size[0]*kernel_size[1] - self.weight_size))) #masked elements (i.e. zeros of filter)
    ...
    def forward(self, x,):
        ...
        x = F.conv2d(input=x, weight=filter,
                     bias=self.bias,
                     stride=self.stride,
                     padding= (self.kernel_size[0]//2,self.kernel_size[1]//2)) # Same padding to compensate for reduced size via filter
        return x
...
class PixelBlock(nn.Module):
    def __init__(self,in_channel,value_size = 80, n_res_block=4, shape=(32,32), dropout_p=0.1,cond_channels=None,
        ...
        for _ in range(n_res_block):
            ...
            h_resblocks.append(GatedResNet(in_channels = in_channel, kernel_size = [2,2], #2x2 causal convolution
                                           n_channels= in_channel, cond_channels=cond_channels,
    ...
    def forward(self, input, background):
        ...
        for v_resblock, h_resblock in zip(self.v_resblocks,self.h_resblocks):
            ...
            h_out = h_resblock(h_out, aux = down_shift(v_out), ) #used to calculate the keys
